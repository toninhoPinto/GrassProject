Shader "Custom/AnimatedWavyGeometry"
{
	Properties
	{
	...
	void geom(triangle v2g IN[3], inout TriangleStream<g2f> triStream) {
		...
		v0.xyz += normal * _Size * max(sin(v0.x * 10 + _Time.x * 100), 0);
		v1.xyz += normal * _Size * max(sin(v1.x * 10 + _Time.x * 100), 0);
		v2.xyz += normal * _Size * max(sin(v2.x * 10 + _Time.x * 100), 0);
